...
(function() {
    ...
    console.log("API Productos:", window.API_PRODUCTOS);
    console.log("API Categorías:", window.API_CATEGORIAS);
    console.log("API Proveedores:", window.API_PROVEEDORES);
        ...
        tbody.innerHTML = `
            <tr>
                <td colspan="11" class="text-center text-danger py-4">
                    ...
                    <small>URL: ${window.API_PRODUCTOS}</small>
                </td>
            </tr>`;
    }
}
...
function llenarSelectCategorias(categorias) {
    ...
    const selectFiltroTicket = document.getElementById('filtro_categoria_print');

   
    if (selectModal) {
        selectModal.innerHTML = '<option value="">Seleccionar categoría...</option>';
        categorias.forEach(cat => {
            const nombre = cat.cat_nombre || `Categoría ${cat.id_categoria}`;
    ...
    if (selectFiltroTicket) {
        const valorActual = selectFiltroTicket.value; 
        selectFiltroTicket.innerHTML = '<option value="">-- Todas las categorías --</option>';
        categorias.forEach(cat => {
            const nombre = cat.cat_nombre || `Categoría ${cat.id_categoria}`;
            const opt = document.createElement('option');
            opt.value = cat.id_categoria;
            opt.textContent = nombre;
            selectFiltroTicket.appendChild(opt);
        });
      
        if (valorActual) {
            selectFiltroTicket.value = valorActual;
        }
    }
...
function cargarProveedores() {
    console.log("📥 Cargando proveedores desde:", window.API_PROVEEDORES);
    ...
    fetch(window.API_PROVEEDORES)
        ...
        .then(response => {
            ...
            console.log(`Proveedores procesados: ${proveedores.length}`);
        ...
        .catch(err => {
            ...
            if (window._proveedoresCache && window._proveedoresCache.length > 0) {
                console.log("Usando caché de proveedores");
                llenarSelectProveedores(window._proveedoresCache);
            }
        });
}
...
function llenarSelectProveedores(proveedores) {
    ...
    proveedores.forEach(prov => {
        ...
        option.textContent = prov.prov_nombre || `Proveedor ${prov.id_proveedor}`;
...
function limpiarFormulario() {
    ['producto_codigo', 'producto_nombre', 'producto_tipo', 'producto_marca',
     'producto_descripcion', 'producto_precio', 'producto_stock',
     'producto_categoria', 'producto_proveedor'].forEach(id => {
...
window.guardarProducto = function() {
    ...
    const producto = {
        ...
        pro_marca: document.getElementById('producto_marca')?.value?.trim() || null,
    ...
    console.log(`Guardando: ${method} ${url}`, producto);
    ...
    Swal.fire({
        ...
        html: `
            <div style="text-align:left;">
                ...
                <p><strong>Marca:</strong> ${p.pro_marca || '-'}</p>
                ...
                <p><strong>Descripción:</strong> ${p.pro_descripcion || '-'}</p>
            </div>`,
        icon: 'info',
...
window.imprimirInventarioTicket = function() {
    ...
    if (productosAImprimir.length === 0) {
        Swal.fire('Atención', 'No hay productos en esta selección', 'info');
        return;
    }
    ...
    doc.text("INVENTARIO FÍSICO", 40, y, { align: "center" });
    y += 5;
    
    doc.setFontSize(7);
    doc.text(`Categoria: ${categoriaNombre}`, 40, y, { align: "center" });
    y += 3.5;
    doc.text(`Fecha: ${new Date().toLocaleDateString('es-MX')}`, 40, y, { align: "center" });
    y += 4;
    ...
    for (const categoria in grupos) {
        ...
        if (y + espacioNecesario > altoFinal - 15) {
            doc.addPage([80, altoFinal]);
            y = 8;
        }
        ...
        doc.setFontSize(8);
        doc.setFont("helvetica", "bold");
        doc.text(categoria.toUpperCase(), 5, y);
        y += 4;
    ...
    doc.setFontSize(8);
    doc.setFont("helvetica", "bold");
    doc.text(`Total productos: ${productosAImprimir.length}`, 40, y, { align: "center" });
    y += 7;
    
    doc.setFontSize(7);
    doc.setFont("helvetica", "normal");
    doc.text("_________________________", 40, y, { align: "center" });
    y += 4;
