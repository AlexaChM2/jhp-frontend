(function() {
    ...
    const CONFIG = {
        ...
        Empleados: {
            ...
            formatearFila: (reg) => `
                ...
                <td><span class="badge bg-${reg.emp_rol === 'Administrador' ? 'danger' : (reg.emp_rol === 'Vendedor' ? 'primary' : 'info')}">${reg.emp_rol || 'N/A'}</span></td>
                <td>${reg.emp_correo || 'N/A'}</td>
                <td><span class="badge bg-${reg.emp_estado === 'Activo' ? 'success' : 'secondary'}">${reg.emp_estado || 'N/A'}</span></td>
            `
        }
    };
    ...
    async function abrirModalVisitas(idProveedor, nombreProveedor) {
        ...
        let visitasHTML = visitas.length > 0
            ? visitas.map(v => `
                <div class="d-flex align-items-center justify-content-between bg-light p-2 mb-1 rounded">
                    <span><strong>${DIAS_SEMANA[v.dia_semana]?.nombre || '?'}</strong> a las ${v.hora_visita?.substring(0, 5)}</span>
                    <button class="btn btn-sm btn-danger" onclick="window.eliminarVisita(${v.id_visita}, ${idProveedor})"><i class="fas fa-trash"></i></button>
                </div>`).join('')
        ...
        const { value: formValues } = await Swal.fire({
            ...
            html: `
                <div class="mb-3"><label class="form-label fw-bold">Días programados:</label>
                <div id="listaVisitasActuales" style="max-height:150px;overflow-y:auto;">${visitasHTML}</div></div>
                <hr><p class="fw-bold mb-2">Agregar día:</p>
                <div class="row">
                    <div class="col-7"><select id="swal-dia-semana" class="form-select">${DIAS_SEMANA.map(d => `<option value="${d.valor}">${d.nombre}</option>`).join('')}</select></div>
                    <div class="col-5"><input type="time" id="swal-hora-visita" class="form-control" value="09:00"></div>
                </div>
                <input type="hidden" id="swal-id-proveedor" value="${idProveedor}">`,
            showCancelButton: true, confirmButtonText: 'Agregar día', cancelButtonText: 'Cerrar', confirmButtonColor: '#28a745',
            preConfirm: async () => {
    ...
    function abrirModalRegistro() {
        editandoID = null;
        document.getElementById("modalTitulo").innerText = "Nuevo " + seccionActual.slice(0, -1);
    ...
    function generarInputs(datos = null) {
        ...
        config.campos.forEach((campo, index) => {
            ...
            if (campo === "emp_rol") {
                ...
                inputHtml = `<input type="password" class="form-control" name="${campo}" placeholder="${editandoID ? 'Repetir nueva contraseña' : 'Repetir contraseña'}" ${required} minlength="6" maxlength="50">`;
            } else if (campo.includes("password")) {
                const required = editandoID ? '' : 'required';
                inputHtml = `<input type="password" class="form-control" name="${campo}" placeholder="${editandoID ? 'Dejar vacío para no cambiar' : 'Mínimo 6 caracteres'}" ${required} minlength="6" maxlength="50">
                ...
                inputHtml = `<input type="text" class="form-control" name="${campo}" value="${valor}">`;
            }
            contenedor.innerHTML += `<div class="mb-3"><label class="form-label small fw-bold">${config.labels[index]}</label>${inputHtml}</div>`;
        });
    }
    ...
    function guardarRegistro(e) {
        ...
        Object.keys(objeto).forEach(key => { if (objeto[key] === '' && key !== 'cli_estado' && key !== 'emp_estado') delete objeto[key]; });
        if (editandoID && (!objeto[passwordField] || objeto[passwordField] === '')) { delete objeto[passwordField]; delete objeto[confirmField]; }
    ...
    function prepararEdicion(reg) {
        ...
        document.getElementById("modalTitulo").innerText = "Editar " + seccionActual.slice(0, -1);
        const modal = document.getElementById("modalCatalogo");
        if (modal) { modal.style.display = "flex"; setTimeout(() => generarInputs(reg), 100); }
    }

    function eliminarRegistro(id, reg) {
        if (seccionActual === 'Empleados' && reg?.emp_rol === 'Administrador') {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                .then(() => { Swal.fire({ icon: 'success', title: 'Eliminado', timer: 1500, showConfirmButton: false }); listarRegistros(); })
    ...
    function cerrarModalCatalogo() {
        const modal = document.getElementById("modalCatalogo"); if (modal) modal.style.display = "none";
        const form = document.getElementById("formCatalogo"); if (form) form.reset();
        editandoID = null;
    }
